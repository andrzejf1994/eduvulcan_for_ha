...
class HttpClient:
    ...
    def _build_body(self, envelope: any) -> str:
        now = datetime.now()
        return json.dumps(
            {
    ...
    async def request(
        ...
        except Exception as exception:  # noqa: BLE001 - keep parity with iris
            raise FailedRequestException(exception) from exception
